...
- в `apps/orders` реализован гостевой checkout: контактные данные,
...
- в `apps/accounts` реализованы регистрация, вход, выход, редактирование
  ...
  доставки, которые подставляются в checkout вместе с именем и обновляются
  после успешного заказа авторизованного пользователя;
- гостевой checkout требует имя и телефон, а email и адрес оставляет
  необязательными;
  регистрация требует email и пароль, принимает необязательный телефон, вход
  выполняется по email без отдельного username в пользовательском интерфейсе;
  фамилия в пользовательском интерфейсе отсутствует, SMS-подтверждение
  телефона в MVP не используется;
...
### Перезапуск локального Django-сервера

- Запускайте только один экземпляр `runserver` и останавливайте его через
  `Ctrl+C` перед повторным запуском.
- Не оставляйте скрытые `Start-Process`, PowerShell или Poetry-цепочки с
  `runserver` на одном порту. Несколько ожидающих процессов могут по очереди
  занимать освободившийся порт 8000 и отдавать ранее загруженный код.
- Если Django shell показывает новое поведение, а `127.0.0.1:8000` продолжает
  отдавать старый HTML, сначала проверяйте владельца порта, а не кэш браузера:
...
- Проверяйте всю родительскую цепочку процесса: дочерний `python.exe` может
  принадлежать старому `poetry run` или скрытому PowerShell-процессу. Перед
  остановкой обязательно сверяйте `CommandLine` и не завершайте чужие процессы.
- После подтверждения цели остановите всё дерево конкретного старого сервера,
  затем убедитесь, что порт свободен, и запустите один новый `runserver`:

```powershell
taskkill /PID <ПОДТВЕРЖДЁННЫЙ_КОРНЕВОЙ_PID> /T /F
poetry run python manage.py runserver
```
...
- В checkout имя и телефон обязательны для гостя и авторизованного
  пользователя. Email и адрес доставки необязательны в обоих случаях. Пустой
  адрес сохраняется в заказе как пустая строка и не препятствует созданию
  заказа.
- Для авторизованного пользователя телефон и адрес доставки хранятся в
  `accounts.Profile`, а имя — в `User.first_name`. Checkout использует их как
  начальные значения и запоминает новые значения только после успешного
  оформления заказа. Это правило применяется и к первому заказу, чтобы
  введённые имя, телефон и адрес появились в профиле и подставлялись в
  последующие заказы.
- Фамилия в пользовательском интерфейсе не используется: отдельного поля
  фамилии нет в регистрации, checkout, редактировании профиля и кабинете.
